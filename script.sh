...
while [ $counter -lt $jobnum ]; do
  echo "ffmpeg -hide_banner -i "$1" -filter:v $cropdetect -strict -1 -f yuv4mpegpipe - | x265 - --no-open-gop --chunk-start " $startframe" --colorprim bt709 --transfer bt709 --colormatrix bt709 --crf=20 --fps 24000/1001 --min-keyint 24 --keyint 240 --chunk-end " $endframe " --sar 1:1 --preset slow --ctu 16 --y4m --pools "+" -o chunky"$counter".265"
